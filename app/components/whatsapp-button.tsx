...
export function WhatsappButton() {
  ...
  const phoneNumber = "5215512345678" // Reemplaza con tu número de WhatsApp real
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {isOpen && (
        <Card className="mb-4 w-64 shadow-lg">
          ...
          <CardFooter className="bg-primary/5 pt-0">
            <Button variant="default" size="sm" className="w-full" onClick={() => window.open(whatsappUrl, "_blank")}>
      ...
      <Button className="rounded-full h-14 w-14 shadow-lg bg-green-500 hover:bg-green-600" onClick={handleClick}>
        <MessageSquare className="h-6 w-6 text-white" />
        <span className="sr-only">{language === "es" ? "Contactar por WhatsApp" : "Contact via WhatsApp"}</span>
      </Button>
    </div>
  )
}
